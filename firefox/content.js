...
const fidoResponseName = "com.siemens.dxa.applications.web.authn.challenging.response";
const fidoOtpText = /(?:one[- ]time(?:\s+(?:password|code))?|totp|otp|2fa|two[- ]factor|second[- ]factor|passcode|verification code|security code|authentication code|authenticator code|token code)/i;

function describeFidoField(field) {
  const label = field.id
    ? document.querySelector(`label[for="${CSS.escape(field.id)}"]`)?.textContent
    : "";
  return [field.name, field.id, field.autocomplete, field.placeholder,
    field.getAttribute("aria-label"), field.getAttribute("label"), label]
    .filter(Boolean).join(" ");
}

function findFidoTotpField() {
  const usable = Array.from(document.querySelectorAll("input")).filter((field) => {
    ...
    return field.autocomplete !== "current-password" && fidoOtpText.test(describeFidoField(field));
  });
  const explicit = usable.find((field) =>
    field.name === `${fidoResponseName}2` ||
    field.autocomplete === "one-time-code" ||
    fidoOtpText.test(describeFidoField(field))
  );
  if (explicit) return explicit;

  const pageText = document.body?.innerText || "";
  return usable.find((field) => field.name === fidoResponseName && fidoOtpText.test(pageText)) || null;
}

function fillAndSubmitTotp(field, form, code) {
  ...
  const ownerForm = field.form || form || field.closest("form");
  if (!ownerForm) return false;
  if (typeof ownerForm.requestSubmit === "function") ownerForm.requestSubmit();
  else ownerForm.submit();
  return true;
}
...
const initializeEthExtension = () => {
      ...
      (async () => {
        try {
          ...
          if (response && response.code) {
            ...
            if (fillAndSubmitTotp(field, form, response.code)) {
              active = false;
  ...
  if (location.href.includes("https://access.ethz.ch/fidoapp/")) {
    console.log("ETH Extension: Processing access.ethz.ch fidoapp page");
    field = findFidoTotpField();
    if (field) {
      processTOTP(field, field.form || field.closest("form"));
    } else {
      console.log("ETH Extension: No OTP field found; leaving this step unchanged");
    }
  } else if (location.href.includes("https://access.ethz.ch/idpauthapp")) {
    ...
    console.log("ETH Extension: - https://access.ethz.ch/fidoapp/");
    console.log("ETH Extension: - https://access.ethz.ch/idpauthapp");
    console.log("ETH Extension: - https://idbdfedin16.ethz.ch");
  }
};

if (document.readyState === "complete") initializeEthExtension();
else window.addEventListener("load", initializeEthExtension, { once: true });
