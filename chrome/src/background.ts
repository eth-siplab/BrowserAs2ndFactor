...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "generateTOTP") {
    chrome.storage.local.get("totpSeed", async (data) => {
      if (typeof data.totpSeed !== "string" || !data.totpSeed) {
        sendResponse({ code: "no_seed" });
        return;
      }
...
chrome.runtime.onInstalled.addListener((details) => {
  // Updates (and browser updates) must never interrupt the user. Only prompt on
  // the first installation, and only when setup has not already completed.
  if (details.reason !== "install") return;

  chrome.storage.local.get("totpSeed", (data) => {
    if (data.totpSeed) return;

    chrome.action.openPopup().catch(() => {
      // Some Chrome versions/window states do not allow a popup to be opened
      // programmatically. The toolbar button remains available for setup.
    });
  });
});
